...
public class GreedyBestFirstSearch<S, A> implements InformedSearchAlgorithm<S, A> {
	private final SearchBase<S, A> searchImp;
	
	public GreedyBestFirstSearch(SearchBase<S, A> searchImp) {
	...
	private static <S, A> Comparator<NodeWrapper<S, A>> makeComparator(Heuristic<S> h) {
		return new Comparator<NodeWrapper<S, A>>() {
			...
			public int compare(NodeWrapper<S, A> o1, NodeWrapper<S, A> o2) {
				return Double.compare(h.estimatedCostAt(o1.node.state), h.estimatedCostAt(o2.node.state));
			}
